...
export async function graphRequest<T>(options: GraphRequestOptions): Promise<T> {
  ...
  if (response.status === 401) {
    ...
    await signOut();
    throw new AppError(USER_MESSAGES.SESSION_EXPIRED, `401 on ${url}`, undefined, 401);
  }

  if (response.status === 404) {
    logger.warn('Graph 404', { url });
    throw new AppError(USER_MESSAGES.FOLDER_NOT_FOUND, `404 on ${url}`, undefined, 404);
  }

  if (response.status === 403) {
    logger.warn('Graph 403 — consentement administrateur requis', { url });
    throw new AppError(USER_MESSAGES.AUTHORIZATION_PENDING, `403 on ${url}`, undefined, 403);
  }
